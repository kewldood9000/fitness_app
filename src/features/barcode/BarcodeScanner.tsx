...
export function BarcodeScanner({ onDetected, onClose }: BarcodeScannerProps) {
  ...
  return <div aria-modal="true" className="modal-backdrop" role="dialog"><div className="modal-panel overflow-hidden"><div className="flex items-center justify-between px-5 pb-3 pt-5"><div><p className="eyebrow">Camera</p><h2 className="mt-1 text-lg font-semibold text-slate-50">Scan food barcode</h2></div><button aria-label="Close scanner" className="workout-icon-button" onClick={onClose}><X className="size-4" /></button></div><div className="px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]"><div className="relative overflow-hidden rounded-2xl bg-slate-950 aspect-[4/3]"><video autoPlay className="h-full w-full object-cover" muted playsInline ref={videoRef} /><div className="pointer-events-none absolute inset-[18%_10%] rounded-xl border-2 border-sky-300/80 shadow-[0_0_0_999px_rgba(2,6,23,0.32)]" /></div>{error && <p className="mt-3 rounded-xl bg-amber-300/10 px-3 py-2.5 text-sm leading-5 text-amber-100">{error}</p>}<div className="mt-3 flex gap-2">{torchAvailable && <button className="button-secondary flex-1" onClick={() => void toggleTorch()}><Flashlight className="size-4" />{torchOn ? 'Torch on' : 'Torch off'}</button>}<span className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-slate-800 px-3 text-xs font-semibold text-slate-400"><Camera className="size-4" />Point at UPC or EAN</span></div><div className="mt-4 border-t border-white/[0.07] pt-4"><label className="field-label">Manual barcode entry<div className="mt-1 flex gap-2"><input className="field-input mt-0" inputMode="numeric" onChange={(event) => setManual(event.target.value.replace(/\D/g, ''))} placeholder="UPC / EAN" value={manual} /><button className="button-primary shrink-0" disabled={!manual} onClick={() => onDetected(manual)}><Keyboard className="size-4" />Use</button></div></label></div></div></div></div>
}
