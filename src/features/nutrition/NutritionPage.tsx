...
function Sheet({ title, onClose, children, headerActions, fullHeight = false, hidden = false, keyboardReflow = false }: { title: string; onClose: () => void; children: ReactNode; headerActions?: ReactNode; fullHeight?: boolean; hidden?: boolean; keyboardReflow?: boolean }) {
  const titleId = useId()
  return createPortal(<div aria-hidden={hidden || undefined} aria-labelledby={titleId} aria-modal={hidden ? undefined : 'true'} className={`modal-backdrop ${hidden ? 'modal-hidden' : ''} ${fullHeight ? 'persistent-modal-backdrop' : ''} ${keyboardReflow ? 'keyboard-reflow-modal' : ''}`} role="dialog"><div className={`modal-panel ${fullHeight ? 'persistent-modal-panel' : ''}`}><div className="modal-header flex items-center justify-between gap-3 px-5 pb-4 pt-5"><h2 className="text-lg font-semibold tracking-[-0.03em] text-slate-50" id={titleId}>{title}</h2><div className="flex items-center gap-1">{headerActions}<button aria-label="Close" className="workout-icon-button" onClick={onClose}><X className="size-4" /></button></div></div><div className="modal-scroll px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]">{children}</div></div></div>, document.body)
}
...
function SavedMealLogSheet({ date, meal, savedMeal, onBack, onEdit, onSaved }: { date: string; meal: Meal; savedMeal: SavedMealDetails; onBack: () => void; onEdit: () => void; onSaved: () => void }) {
  const [grams, setGrams] = useState(() => Object.fromEntries(savedMeal.items.map((item) => [item.id, ''])))
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  async function logMeal(useSuggested = false) {
    const inputs = savedMeal.items.map((item) => ({ item, grams: useSuggested ? item.defaultGrams : grams[item.id].trim() ? Number(grams[item.id]) : item.defaultGrams })).filter(({ grams }) => Number.isFinite(grams) && grams > 0)
    if (!inputs.length) return setError('Enter a weight for at least one food.')
    setSaving(true)
    try { await nutritionRepository.logFoods(inputs.map(({ item, grams }) => ({ date, meal, foodId: item.foodId, quantity: grams, amountUnit: 'g' }))); await nutritionRepository.markSavedMealUsed(savedMeal.meal.id); onSaved() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to log this meal.') } finally { setSaving(false) }
  }
  return <Sheet fullHeight headerActions={<button aria-label={`Edit ${savedMeal.meal.name}`} className="workout-icon-button" onClick={onEdit} type="button"><Settings className="size-4" /></button>} keyboardReflow onClose={onBack} title={savedMeal.meal.name}><div className="space-y-2">{savedMeal.items.map((item) => { const amount = grams[item.id].trim() ? Number(grams[item.id]) || 0 : item.defaultGrams; const factor = amount / 100; return <div className="flex items-center gap-3 rounded-xl bg-slate-800/65 px-4 py-3" key={item.id}><div className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-100">{foodDisplayName(item.food.food)}</strong><small>{Math.round(item.food.nutrients.ENERGY_KCAL * factor)} kcal · {Math.round(item.food.nutrients.PROTEIN * factor * 10) / 10}g protein</small></div><label className="field-label w-24 shrink-0">Grams<input aria-label={`${foodDisplayName(item.food.food)} grams`} className="field-input mt-1" inputMode="decimal" min="0" onChange={(event) => setGrams((current) => ({ ...current, [item.id]: event.target.value }))} placeholder={String(item.defaultGrams)} step="any" type="number" value={grams[item.id]} /></label></div> })}</div>{error && <p className="mt-3 text-sm text-rose-300">{error}</p>}<div className="mt-5 grid grid-cols-2 gap-2"><button className="button-secondary" disabled={saving} onClick={() => void logMeal(true)}>{saving ? 'Logging…' : 'Log suggested'}</button><button className="button-primary" disabled={saving} onClick={() => void logMeal()}>{saving ? 'Logging…' : 'Done — log foods'}</button></div></Sheet>
}
