...
function ExercisePicker({ onPick, onClose }: { onPick: (exerciseId: string) => Promise<void>; onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} title="Add exercise">
      <input autoFocus className="field-input mb-3" onChange={(event) => setQuery(event.target.value)} placeholder="Search your exercise library" type="search" value={query} />
      {filtered.length === 0 ? (
...
export function WorkoutHubPage() {
  ...
  return (
    <div className="space-y-6 pb-3 pt-2">
      <PageHeader detail="Templates and sessions stay fully on this device." eyebrow="Training" title="Your workout week" />
      ...
      <section className="overflow-hidden rounded-[1.4rem] border border-white/[0.07] bg-slate-900/75 shadow-card">
        {[
          ['Workout templates', 'Create reusable training sessions', LibraryBig, '/workout/templates'],
...
export function ExerciseLibraryPage() {
  ...
  async function remove(exercise: Exercise) {
    if (!window.confirm(`Delete ${exercise.name}? Exercises used by a template or history are protected.`)) return
    try {
...
export function WorkoutSchedulePage() {
  ...
  return <div className="space-y-5 pb-3 pt-2"><BackLink to="/workout">Workout</BackLink><PageHeader eyebrow="Programming" title="Weekly schedule" detail="Assign templates to days—or leave them as rest days." />
    ...
    {(templates?.length ?? 0) === 0 && <div className="rounded-xl border border-amber-300/15 bg-amber-300/[0.07] px-4 py-3 text-sm text-amber-100">Create a workout template before assigning your week.</div>}
  </div>
}
...
function ActiveSetRow({ set, showRir }: { set: WorkoutSet; showRir: boolean }) {
  ...
  return <div className={`active-set-row ${showRir ? '' : 'active-set-row-no-rir'} ${set.completed ? 'active-set-row-complete' : ''}`}><span className="set-number-badge">{set.order + 1}</span><input aria-label={`Set ${set.order + 1} weight`} className="set-input" defaultValue={set.weight ?? ''} inputMode="decimal" onBlur={(event) => saveNumeric('weight', event.target.value)} placeholder="Weight" type="number" /><input aria-label={`Set ${set.order + 1} reps`} className="set-input" defaultValue={set.reps ?? ''} inputMode="numeric" onBlur={(event) => saveNumeric('reps', event.target.value)} placeholder="Reps" type="number" />{showRir && <select aria-label={`Set ${set.order + 1} RIR`} className="set-input" defaultValue={set.rir ?? ''} onChange={(event) => saveNumeric('rir', event.target.value)}><option value="">RIR</option>{[0, 1, 2, 3, 4, 5].map((rir) => <option key={rir} value={rir}>{rir === 5 ? '5+' : rir}</option>)}</select>}<button aria-label={set.completed ? `Mark set ${set.order + 1} incomplete` : `Complete set ${set.order + 1}`} className={`set-done-button ${set.completed ? 'set-done-button-complete' : ''}`} disabled={saving} onClick={() => void complete()}>{set.completed ? <Check className="size-4" /> : 'Done'}</button><button aria-label={`Delete set ${set.order + 1}`} className="set-delete-button" onClick={() => { if (window.confirm('Delete this set?')) void workoutRepository.deleteSet(set.id) }}><X className="size-4" /></button></div>
}
