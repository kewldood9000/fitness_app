...
export function SettingsPage() {
  ...
  return <div className="space-y-5 pb-3 pt-2">
    ...
    <SettingsCard detail="Barcode scans check each enabled database. Credentials and proxy configuration stay in this browser and are excluded from backups." title="Food Data"><div className="rounded-xl border border-emerald-300/15 bg-emerald-300/[0.06] px-3 py-2.5"><p className="text-sm font-semibold text-emerald-100">Open Food Facts enabled</p><p className="mt-1 text-xs leading-5 text-emerald-50/65">No API key is required for barcode lookup.</p></div><div className="mt-4 flex items-center gap-2"><KeyRound className="size-4 text-sky-300" /><label className="field-label min-w-0 flex-1">USDA API key<input className="field-input" defaultValue={credential?.value} onBlur={(event) => void credentialRepository.set('usda-api-key', event.target.value)} placeholder="Paste your data.gov key" type="password" /></label></div><p className="mt-2 text-xs leading-5 text-slate-500">Enables USDA text search and includes USDA in barcode scans.</p><label className="field-label mt-4">FatSecret proxy URL<input className="field-input" defaultValue={fatSecretProxy?.value} inputMode="url" onBlur={(event) => void credentialRepository.set(FATSECRET_PROXY_URL_KEY, event.target.value)} placeholder="https://your-worker.workers.dev" type="url" /></label><p className="mt-2 text-xs leading-5 text-slate-500">FatSecret requires a server-side OAuth proxy so its Client Secret is never exposed in this public PWA. Deploy the included proxy, add its URL here, and FatSecret will join every barcode scan.</p></SettingsCard>
    <BackupRestore />
